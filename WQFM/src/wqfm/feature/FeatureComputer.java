...
//        int[] arr = {tax1, tax2, tax3, tax4};
    ...
    public static void computeBinningFeatureUsingLevel1(Map<List<Integer>, List<Double>> dictionary_4Tax_sequence_weight,
            CustomDSPerLevel customDS) {
        if (customDS.level > 1) {
            System.out.println("Bin only level 1, current level = " + customDS.level + " , not binning, beta = " + customDS.BETA_PARTITION_SCORE);
            return;
        }
        ...
        if (list_ratios.isEmpty()) { // empty ratios -> no 4-tax-seq exists.
            ...
            customDS.ALPHA_PARTITION_SCORE = DefaultValues.ALPHA_DEFAULT_VAL;
            customDS.BETA_PARTITION_SCORE = DefaultValues.BETA_DEFAULT_VAL; // beta default = 1
            if (customDS.level == 1) {
            ...
            if (Bin.WILL_DO_DYNAMIC == true) { //only compute on Bin.true
                ...
                Bin.WILL_DO_DYNAMIC = false;
                customDS.BETA_PARTITION_SCORE = weighted_avg_bin_ratio;
                System.out.println("Bin only level 1, beta = " + customDS.BETA_PARTITION_SCORE);
    ...
    public static void computeBinningFeatureUsingAllLevels(Map<List<Integer>, List<Double>> dictionary_4Tax_sequence_weight,
        ...
        List<Double> list_ratios = new ArrayList<>();
        //double[] alpha_beta_partition_score = {WeightedPartitionScores.ALPHA_PARTITION_SCORE, WeightedPartitionScores.BETA_PARTITION_SCORE};
...
//            printDictionary(dictionary_4Tax_sequence, dictionary_4Tax_sequence_weight);
            ...
            if (customDS.level == 1) { //check on level == 1 and set accordingly.
                if (Bin.proportion_left_thresh < Config.CUT_OFF_LIMIT_BINNING) { //level == 1 has no good distribution ... so do no more.
                    ...
                    customDS.BETA_PARTITION_SCORE = DefaultValues.BETA_DEFAULT_VAL; // set to 1 
                }
            ...
            if (Bin.WILL_DO_DYNAMIC == true) { //only bin on true conditions.
                ...
                System.out.println("Level" + customDS.level + ", Weighted-Avg-Bin-Ratio (to set BETA) = " + weighted_avg_bin_ratio);
...
//        System.out.println("WeightedPartitionScores.BETA_PARTITION_SCORE = "+WeightedPartitionScores.BETA_PARTITION_SCORE);
    ...
    public static void computeBinningFeature(Map<List<Integer>, List<Double>> dictionary_4Tax_sequence_weight,
        ...
        if (Config.BIN_LIMIT_LEVEL_1 == true) {
            ...
            FeatureComputer.computeBinningFeatureUsingLevel1(dictionary_4Tax_sequence_weight, customDS);
        } else {
            System.out.println("Binning on all levels.");
            FeatureComputer.computeBinningFeatureUsingAllLevels(dictionary_4Tax_sequence_weight, customDS);
        }
