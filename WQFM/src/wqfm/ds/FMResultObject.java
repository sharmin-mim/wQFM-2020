...
public class FMResultObject {
    ...
    private final Map<Quartet, CustomPair> map_quartet_of_dummy_with_added_weights_and_counts;//change// consumes lots of memory
    //private final Map<Quartet, double[]> map_quartet_dummy_with_weight_and_counts = new HashMap<>();//change// consumes lots of memory
    ...
    public FMResultObject(CustomDSPerLevel customDS_this_level, int level) {
        ...
        this.customDS_left_partition = new CustomDSPerLevel(this.customDS_initial_this_level.ALPHA_PARTITION_SCORE, this.customDS_initial_this_level.BETA_PARTITION_SCORE); //do not initialize tables YET
        this.customDS_right_partition = new CustomDSPerLevel(this.customDS_initial_this_level.ALPHA_PARTITION_SCORE, this.customDS_initial_this_level.BETA_PARTITION_SCORE); //do not initialize tables YET
        ...
        this.map_quartet_of_dummy_with_added_weights_and_counts = new HashMap<>();
...
//        this.map_quartet_dummy_with_counts = new HashMap<>();
    ...
    public void createFMResultObjects() {
    	//at first clear unneccessay quartets i.e satisfied and violated ones
...
//        
        ...
        for (Quartet quartet_parent : this.customDS_initial_this_level.initial_table1_of_list_of_quartets.list_quartets) {
//			
//		}
//        for (int itr = 0; itr < this.customDS_initial_this_level.initial_table1_of_list_of_quartets.sizeTable(); itr++) {
//           // int qrt_idx = this.customDS_initial_this_level.quartet_indices_list_unsorted.get(itr); //add to new lists of customDS
...
//            System.out.println("quartet_parent.taxa_sisters_left[0].taxa_int_name = "+quartet_parent.taxa_sisters_left[0].taxa_int_name);
            int left_1_partition = this.customDS_initial_this_level.map_of_int_vs_tax_property.get(quartet_parent.taxa_sisters_left[0]).prev_partition;
...
//                    	.add(this.customDS_right_partition.initial_table1_of_list_of_quartets.sizeTable() - 1);
            	...
            	int left_2_partition = this.customDS_initial_this_level.map_of_int_vs_tax_property.get(quartet_parent.taxa_sisters_left[1]).prev_partition;
                int right_1_partition = this.customDS_initial_this_level.map_of_int_vs_tax_property.get(quartet_parent.taxa_sisters_right[0]).prev_partition;
                int right_2_partition = this.customDS_initial_this_level.map_of_int_vs_tax_property.get(quartet_parent.taxa_sisters_right[1]).prev_partition;
...
//    			quartet_parent.taxa_sisters_right[0].get_taxa_int_name()+","+quartet_parent.taxa_sisters_right[1].get_taxa_int_name());
                ...
                System.out.println(newQuartetWithDummy.toString());
                // do not add yet, first put to map with added weight eg. 1,2|5,11 and 1,2|5,15 will be 1,2|5,X with weight = w1+w2
                if (this.map_quartet_of_dummy_with_added_weights_and_counts.containsKey(newQuartetWithDummy) == false) { //this quartet-of-dummy DOESN't exist.
                    this.map_quartet_of_dummy_with_added_weights_and_counts.put(newQuartetWithDummy, new CustomPair(newQuartetWithDummy.weight,1));//, commonBipartitionValue)); //initialize with 0 so that next step doesn't have to be if-else
//                    this.map_quartet_dummy_with_counts.put(newQuartetWithDummy, 1); // current count = 1
//                    double[] weight_count_array = {newQuartetWithDummy.weight,1};
//                    this.map_quartet_dummy_with_weight_and_counts.put(newQuartetWithDummy, weight_count_array);
                } else {
                    // else we will add weights for the Pair (value of the map_quartet_of_dummy_with_added_weights_and_partition)
                	//System.out.println(newQuartetWithDummy.toString());
                    CustomPair pair_value_from_map = this.map_quartet_of_dummy_with_added_weights_and_counts.get(newQuartetWithDummy);
                    int count = pair_value_from_map.num_times;//this.map_quartet_dummy_with_counts.get(newQuartetWithDummy);
//                    double[] weight_count_array = this.map_quartet_dummy_with_weight_and_counts.get(newQuartetWithDummy);
//                    int count1 = (int) weight_count_array[1];
                    double newWeight;
                    ...
                    CustomPair new_pair = new CustomPair(newWeight, count + 1);//, pair_value_from_map.partition_int);
                    //this will update the added weights while maintaining the same bipartition.
                    this.map_quartet_of_dummy_with_added_weights_and_counts.put(newQuartetWithDummy, new_pair);
...
//        System.out.println("this.customDS_left ListOfQuartets= "+this.customDS_left_partition.initial_table1_of_list_of_quartets.sizeTable());
//        System.out.println("this.customDS_right ListOfQuartets= "+this.customDS_right_partition.initial_table1_of_list_of_quartets.sizeTable());
...
//        });
        ...
        this.map_quartet_of_dummy_with_added_weights_and_counts.keySet().forEach((q_with_dummy) -> {
            CustomPair pair_val = this.map_quartet_of_dummy_with_added_weights_and_counts.get(q_with_dummy);
...
//                    pair_val.weight_double); //update the weight now.
            q_with_dummy.weight = pair_val.weight_double;
            //System.out.println(q_with_dummy.toString()+".............newQ from deffered");
...
//            }
            //in this level int_value_of_dummy_taxon > int_value_of_other_sister_leaves. so dummy taxon won't be in left_sis[0] or in right_sis[0] index as indices are sorted
            if (this.customDS_initial_this_level.map_of_int_vs_tax_property.get(q_with_dummy.taxa_sisters_left[0]).prev_partition == DefaultValues.LEFT_PARTITION) {
            	this.customDS_left_partition.initial_table1_of_list_of_quartets.addToListOfQuartets(q_with_dummy);
			} else {
				this.customDS_right_partition.initial_table1_of_list_of_quartets.addToListOfQuartets(q_with_dummy);
			}
        });
//        System.out.println("size = "+this.map_quartet_of_dummy_with_added_weights_and_counts.size());
        this.map_quartet_of_dummy_with_added_weights_and_counts.clear();
//        System.out.println("this.customDS_left ListOfQuartets= "+this.customDS_left_partition.initial_table1_of_list_of_quartets.sizeTable());
//        System.out.println("this.customDS_right ListOfQuartets= "+this.customDS_right_partition.initial_table1_of_list_of_quartets.sizeTable());
        ...
        this.customDS_initial_this_level.map_of_int_vs_tax_property.keySet().forEach((key_taxon) -> {
        	Taxa taxon = this.customDS_initial_this_level.map_of_int_vs_tax_property.get(key_taxon);
        	if (taxon.prev_partition == DefaultValues.LEFT_PARTITION) {
        		this.customDS_left_partition.map_of_int_vs_tax_property.put(key_taxon, taxon);
			} else {
				this.customDS_right_partition.map_of_int_vs_tax_property.put(key_taxon, taxon);
			}
        	taxon.reset_tax_property();
        });

        if (this.customDS_initial_this_level.level != 1) {
            this.customDS_initial_this_level.map_of_int_vs_tax_property.clear();
		}
